...
| ADR-015 | Agent-memory integration — ProximaKit as tinybrain's on-device memory substrate: store-level auto-checkpoint hook (`checkpointAutomatically:`, closes `api-ergo-01`), paged dense leg in journaled stores (M5-F49 `dense:` param), `HNSWIndex.load(from:mode:)` mirror (`api-ergo-02`), canonical `IndexResidency` / `IndexSaveLayout` naming (`api-ergo-03`), and the documented one-store plus optional consumer-composed hot/cold patterns ([ADR-015](adr/ADR-015-agent-memory-integration.md)) | **Accepted — Stages A+B+C implemented for v1.9**; `HNSWOpenMode` / `PQHWOpenMode` / `PQHWSaveLayout` remain deprecated source-compatible aliases, with no persistence migration |
...
| Index inspector — visualise the HNSW layer graph as a force-directed diagram | **Shipped** — force-directed visualization of the real HNSW graph (Canvas/TimelineView, seeded deterministic layout, pausable), layer histogram, tap-to-select showing the node's document text and metadata, driven by the non-mutating public `liveGraphSnapshot()` |
...
- CoreML model install instructions — **shipped**: verified setup and explicit model/vocabulary URL guidance in the README
