...
- Uses the public `HNSWIndex.liveGraphSnapshot()` API to read live node ids, node levels, metadata, and live layer-0 adjacency without compacting or materializing vectors
...
2. The engine calls the public `HNSWIndex.liveGraphSnapshot()` actor method and projects its live nodes, node levels, metadata, and live layer-0 neighbor UUIDs into display nodes and sampled links.
