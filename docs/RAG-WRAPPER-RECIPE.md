...
ProximaKit ships two layers you can build RAG on. `VectorStore` / `HybridVectorStore` (see [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) → Store Layer) manage the document→chunk map for you and are the right default when you want text in, answers out. For the recommended agent-memory store pattern, paging, metadata filters, and the mechanism/policy boundary, start with the [Agent Memory DocC guide](../Sources/ProximaKit/Documentation.docc/AgentMemory.md). This recipe stays focused on consumers that already own a chunking pipeline — sentence splitters, overlap windows, per-source provenance, their own IDs — and want to drop embeddings straight into the graph and get their own records back on search. **Wrapping `HNSWIndex` directly is a first-class path, not an advanced hack.** This recipe is how you do it without giving up crash safety.
...
**The do-not-retry contract — restate it and obey it.** A failed automatic store fold is rethrown by the mutation call that triggered it, but the triggering mutation may already be applied and durable — including when the failure occurs during post-commit sidecar-cache refresh. Do not retry `addChunks`: it assigns fresh UUIDs, so retrying can duplicate chunks. The store remains consistent; a later mutation can attempt the automatic fold again, and an explicit `checkpoint()` can be retried. Journaled `save()` only synchronizes the WAL and does **not** retry the fold.

At the raw-index level, if a journaled `add` or a checkpoint throws, **do not blindly retry it.** The vector may already be present in memory or durable in the WAL; retrying with a fresh `UUID` can duplicate the chunk. If you must retry, do so only under an idempotency policy *you* own (for example, a stable content-derived `id` you can deduplicate), or reopen and reconcile from the recovered metadata.
...
- [Agent Memory DocC guide](../Sources/ProximaKit/Documentation.docc/AgentMemory.md) — the recommended one-store pattern, hybrid recall, paging, policy metadata, and optional consumer-composed hot/cold tiering
